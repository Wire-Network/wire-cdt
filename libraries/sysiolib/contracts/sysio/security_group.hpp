...
extern "C" {
__attribute__((sysio_wasm_import)) int64_t add_security_group_participants(const char* data, uint32_t datalen);

__attribute__((sysio_wasm_import)) int64_t remove_security_group_participants(const char* data, uint32_t datalen);

__attribute__((sysio_wasm_import)) bool in_active_security_group(const char* data, uint32_t datalen);

__attribute__((sysio_wasm_import)) uint32_t get_active_security_group(char* data, uint32_t datalen);
}
} // namespace internal_use_do_not_use
...
inline int64_t add_security_group_participants(const std::set<name>& participants) {
   auto packed_participants = sysio::pack( participants );
...
inline security_group get_active_security_group() {
   ...
   return sysio::unpack<security_group>(buffer);
}
} // namespace sysio