#pragma once
/**
 * kv::indexed_table -- Format=0 ordered key-value table with secondary indexes.
 *
 * Combines the compact format=0 raw byte keys of kv::raw_table with
 * kv_multi_index-style secondary indexes.  No legacy scope overhead.
 *
 * WARNING: Primary keys share a SINGLE FLAT NAMESPACE with all format=0 data
 * in the same contract (including other indexed_table and raw_table instances).
 * Use a unique tag/discriminator field in your key struct to prevent collisions.
 * TableName scopes secondary indexes only — it does NOT namespace primary keys.
 *
 * Usage:
 *   struct my_key {
 *      uint64_t id;
...
#include <sysio/kv_raw_table.hpp>   // be_key_stream, kv_format_raw, primary intrinsics
...
// Secondary index intrinsic declarations (not in kv_raw_table.hpp).
...
/// Index declaration for indexed_table (analogous to sysio::indexed_by).
...
class indexed_table {
   static_assert(sizeof...(Indices) <= 16, "indexed_table supports at most 16 secondary indices");
   ...
   void do_erase(const K& key, const V& value) {
      ...
      auto pri = make_key(key);
      ::kv_erase(kv_format_raw, pri.data(), pri.size());
   }

public:
   /// Construct an indexed_table. Reads/iterates against \p code's data (default: current contract).
   indexed_table(sysio::name code = sysio::name{}) : _code(code.value) {}
   ...
   struct const_iterator {
      ...
      const row& operator*()  const { sysio::check(_valid, "deref end iterator"); return _row; }
      const row* operator->() const { sysio::check(_valid, "deref end iterator"); return &_row; }
   ...
   private:
      ...
      static const_iterator make_end(const indexed_table* t) {
   ...
   const_iterator find(const K& key) const {
      auto k = make_key(key);
      if (!::kv_contains(kv_format_raw, code(), k.data(), k.size()))
         return end();
      uint32_t h = ::kv_it_create(kv_format_raw, code(), nullptr, 0);
   ...
   /// Insert a new row. The key must not already exist — calling emplace on an
   /// existing key is undefined behavior (secondary index entries will be
   /// orphaned and unrecoverable). Use upsert() if the key may already exist,
   ...
   struct secondary_index_view {
      ...
      indexed_table* _tbl;
      secondary_index_view(indexed_table& tbl) : _tbl(&tbl) {}
      ...
      struct const_iterator {
         ...
         const row& operator*()  const { sysio::check(_valid, "deref invalid sec iter"); return _row; }
         const row* operator->() const { sysio::check(_valid, "deref invalid sec iter"); return &_row; }
      ...
      private:
         ...
         static const_iterator make_end(indexed_table* tbl) {
      ...
      private:
         ...
         static key_iterator make_end(indexed_table* tbl) {
      ...
      const_iterator begin() const {
         int32_t handle = ::kv_idx_lower_bound(
            _tbl->code(), static_cast<uint64_t>(TableName), index_number, nullptr, 0);
