 ...
 *  @details Database C API provides low level interface to SYSIO database.
 *
...
/**
  ...
  *  sysio_assert(itr >= 0, "Alice cannot be removed since she was already not found in the table");
  *  db_remove_i64(itr);
  *  @endcode
  */
__attribute__((sysio_wasm_import))
...
/**
  ...
  *  sysio_assert(len <= 50, "buffer to small to store retrieved record");
...
/**
  ...
  *  sysio_assert(end_itr < -1, "Charlie was not the last entry in the table");
  *  @endcode
  */
__attribute__((sysio_wasm_import))
