...
#include "core/sysio/powers.hpp"
#include "contracts/sysio/system.hpp"
#include "contracts/sysio/privileged.hpp"
...
namespace sysio {
   ...
   bool block_signing_authority_v0::is_valid()const {
      ...
      std::set<sysio::public_key> unique_keys;
   ...
   // privileged.hpp
   void set_blockchain_parameters(const sysio::blockchain_parameters& params) {
   ...
   void get_blockchain_parameters(sysio::blockchain_parameters& params) {
      char buf[sizeof(sysio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      sysio::check( size <= sizeof(buf), "buffer is too small" );
      sysio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }

   std::optional<uint64_t> set_proposed_producers( const std::vector<producer_key>& prods ) {
      auto packed_prods = sysio::pack( prods );
