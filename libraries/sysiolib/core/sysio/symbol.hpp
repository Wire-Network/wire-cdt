...
namespace sysio {
   ...
   */
   ...
   inline DataStream& operator<<(DataStream& ds, const sysio::symbol_code sym_code) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, sysio::symbol_code& sym_code) {
     uint64_t raw = 0;
   ...
   public:
      ...
      void print( bool show_precision = true )const {
         if( show_precision ){
            ::sysio::print( static_cast<uint64_t>(precision()), "," );
         }
         char buffer[7];
    ...
    */
   template<typename DataStream>
   inline DataStream& operator<<(DataStream& ds, const sysio::symbol sym) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, sysio::symbol& sym) {
     uint64_t raw = 0;
     ds.read((char*)&raw, sizeof(raw));
     sym = symbol(raw);
     return ds;
