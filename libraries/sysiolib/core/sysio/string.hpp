...
#include "datastream.hpp" // sysio::datastream
#include "varint.hpp"     // sysio::unsigned_int
...
namespace sysio {
   ...
   public:
      ...
      char& at(const size_t n) {
         sysio::check(0 <= n && n < _size, "sysio::string::at");
         return operator[](n);
      }

      const char at(const size_t n) const {
         sysio::check(0 <= n && n < _size, "sysio::string::at const");
         return operator[](n);
      }
      ...
      size_t copy(char* s, size_t len, size_t pos = 0) const {
         sysio::check(pos <= _size, "sysio::string::copy");
      ...
      string& insert(const size_t pos, const char* str, const size_t len) {
         sysio::check((str != nullptr) && (0 <= pos && pos <= _size), "sysio::string::insert");
      ...
      string& erase(size_t pos = 0, size_t len = npos) {
         sysio::check(0 <= pos && pos <= _size, "sysio::string::erase");
      ...
      string& append(const char* str) {
         sysio::check(str != nullptr, "sysio::string::append");
         insert(_size, str);
