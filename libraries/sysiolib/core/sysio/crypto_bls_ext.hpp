...
namespace detail {
    ...
    std::string bls_type_to_base64url(const T& g1) {
        ...
        return Prefix + sysio::base64url_encode({g1_with_checksum.data(), g1_with_checksum.size()});
    }

    template<typename T, const std::string& Prefix>
    T bls_base64url_to_type(const char* data, size_t size) {
        sysio::check(size > Prefix.size(), "encoded base64 key is too short");
        sysio::check(0 == memcmp(data, Prefix.data(), Prefix.size()), "base64 encoded type must begin from corresponding prefix");

        std::string decoded = sysio::base64url_decode({data+Prefix.size(), size - Prefix.size()});
        T ret;
        sysio::check(decoded.size() == ret.size() + bls_checksum_size, "decoded size " + std::to_string(decoded.size()) + 
        ...
        sysio::check(0 == memcmp(&*it, csum.data(), bls_checksum_size), "checksum of structure doesn't match");
