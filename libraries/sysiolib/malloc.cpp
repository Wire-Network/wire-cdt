...
namespace sysio {
   ...
   private:
      ...
      public:
         ...
         {
            sysio::check(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
extern "C" {
void* malloc(size_t size) {
   return sysio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size) {
   void* ptr = sysio::memory_heap.malloc(count*size);
   memset(ptr, 0, count*size);
   return ptr;
}

void* realloc(void* ptr, size_t size) {
   return sysio::memory_heap.realloc(ptr, size);
}

void free(void* ptr) {
   return sysio::memory_heap.free(ptr);
}
}
