...
inline bool expect_assert(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   if (ret == 0) {
      ...
      if (!check)
         sysio::check(false, std::string("error : expect_assert, no assert {"+li+"}").c_str());
      sysio::print("error : expect_assert, no assert {"+li+"}\n");
   ...
   silence_output(false);
   if (!check)
      sysio::check(passed, std::string("error : expect_assert, wrong assert {"+li+"}").c_str());
   if (!passed)
      sysio::print("error : expect_assert, wrong assert {"+li+"}\n");
   silence_output(disable_out);
...
inline bool expect_print(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   silence_output(false);
   if (!check)
      sysio::check(passed, std::string("error : wrong print message {"+li+"}").c_str());
   if (!passed)
      sysio::print("error : wrong print message {"+li+"}\n");
   silence_output(disable_out);
   return passed;
}
...
#define CHECK_EQUAL(X, Y) \
   if (!(X == Y)) { \
      ___has_failed = true; \
      sysio::print(std::string("CHECK_EQUAL failed (")+#X+" != "+#Y+") {"+__FILE__+":"+std::to_string(__LINE__)+"}\n"); \
   }
...
#define EOSIO_TEST(X) \
   ...
   if ( X ## _ret == 0 ) \
      ...
      silence_output(false); \
      sysio::print("\033[1;37m", #X, " \033[0;37munit test \033[1;31mfailed\033[0m (aborted)\n"); \
      ___has_failed = true; \
...
#define EOSIO_TEST_END \
      ...
      silence_output(false); \
      if (___has_failed) \
         sysio::print("\033[1;37m", __test_name, " \033[0;37munit test \033[1;31mfailed\033[0m\n"); \
      else \
         sysio::print("\033[1;37m", __test_name, " \033[0;37munit test \033[1;32mpassed\033[0m\n"); \
      silence_output(___original_disable_output); \
