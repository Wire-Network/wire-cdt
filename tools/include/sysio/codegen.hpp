...
namespace sysio { namespace cdt {
   ...
   class sysio_codegen_visitor : public RecursiveASTVisitor<sysio_codegen_visitor>, public generation_utils {
      ...
      public:
         ...
         explicit sysio_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class sysio::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_sysio_contract(decl, cg.contract_name)) {
               ...
               ss << "}\n";
               ss << "sysio::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
      ...
      class sysio_codegen_consumer : public ASTConsumer, public generation_utils {
      private:
         sysio_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit sysio_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new sysio_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  ss << "extern \"C\" {\n";
                  ss << "__attribute__((sysio_wasm_import))\n";
                  ss << "void sysio_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, sysio_wasm_entry, sysio_wasm_abi(";
                  ss << "\"" << quoted(cg.abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_sysio_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
      ...
      class sysio_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(std::make_unique<sysio_ppcallbacks>(CI.getSourceManager(), file.str()));
            return std::make_unique<sysio_codegen_consumer>(&CI, file);
         }
   };
