...
### Working directories
Cases run in parallel (`-j`, default half the CPUs), and each case compiles in its own scratch directory, `<work root>/<suite>/<case>` — for example `/tmp/toolchain-tester-abc123/abigen-pass/named_table_rename_chain_0`. The toolchain names its outputs (`.o`, `.desc`, `.actions.cpp`, `.abi`, `.wasm`) after the source file's basename, so this is what keeps two cases that compile a `second.cpp`, or the three cases of one JSON file, from overwriting each other's artifacts mid-run. A case may therefore never depend on files another case produced.

The directories are kept after the run. When a case fails, its directory is printed under the failure message (`artifacts: …`); the `.abi` it produced and the intermediate `.desc` files are there for comparison against the expected output.
