...
struct generation_utils {
   ...
   // --- Ignorable type checks (using clang_wrapper) ---
      ...
      auto check = [&](const clang::Type* pt) {
        if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
         if (auto rt = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
            return clang_wrapper::wrap_decl(rt->getDecl()).isSysioIgnore();
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return clang_wrapper::wrap_decl(decl->getDecl()).isSysioIgnore() ? tst->template_arguments()[0].getAsType() : type;
         return type;
      };
   ...
   // --- Ricardian helpers (using clang_wrapper) ---
   template<typename T>
   static inline bool has_sysio_ricardian( const clang_wrapper::Decl<T>& decl ) {
      return decl.hasSysioRicardian();
   }

   template<typename T>
   static inline std::string get_sysio_ricardian( const clang_wrapper::Decl<T>& decl ) {
      return decl.getSysioRicardianAttr()->getNameAsString();
   }

   // --- Action/Notify name helpers (using clang_wrapper) ---
   template<typename T>
   static inline std::string get_action_name( const clang_wrapper::Decl<T>& decl ) {
      std::string action_name = "";
      auto tmp = decl.getSysioActionAttr()->getNameAsString();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang_wrapper::Decl<T>& decl ) {
      ...
      auto tmp = decl.getSysioNotifyAttr()->getNameAsString();
      return tmp;
   }
   ...
   // --- Contract detection (using clang_wrapper) ---
   template<typename T>
   static inline bool is_sysio_contract( const clang_wrapper::Decl<T>& decl, const std::string& cn ) {
      std::string name = "";
      if (auto* r = llvm::dyn_cast<clang::CXXRecordDecl>(*decl)) {
         auto pd = decl.getParent();
         if (decl.isSysioContract()) {
            auto nm = decl.getSysioContractAttr()->getNameAsString();
            name = nm.empty() ? r->getName().str() : nm;
         } else if (pd && pd.isSysioContract()) {
            auto nm = pd.getSysioContractAttr()->getNameAsString();
            name = nm.empty() ? pd->getName().str() : nm;
         }
         parsed_contract_name = name;
         // When cn is empty (no explicit --contract), accept any sysio contract class
         return !name.empty() && (cn.empty() || cn == parsed_contract_name);
      } else if (auto* m = llvm::dyn_cast<clang::CXXMethodDecl>(*decl)) {
         if (decl.isSysioContract()) {
            name = decl.getSysioContractAttr()->getNameAsString();
         } else if (decl.getParent().isSysioContract()) {
            name = decl.getParent().getSysioContractAttr()->getNameAsString();
         }
         if (name.empty()) {
   ...
   template<typename T>
   static inline bool is_sysio_contract( T decl, const std::string& cn ) {
      auto _decl = clang_wrapper::wrap_decl(decl);
      return is_sysio_contract(_decl, cn);
   }
      ...
      if (std::holds_alternative<clang::QualType>(arg)) {
         ...
         if (auto ce = llvm::dyn_cast<clang::CastExpr>(std::get<clang::Expr*>(arg))) {
            ...
            return std::to_string(il->getValue().getLimitedValue());
             ...
             ce->getResultAsAPSInt().toString(buf, 10);
             return std::string(buf.str());
         }
      } else {
         llvm::SmallString<40> buf;
         std::get<llvm::APSInt>(arg).toString(buf, 10);
         return std::string(buf.str());
      }
   ...
   // return combined typename, only be used on explicit nested type which has <<>> or more
   ...
   /// The other namespace a builtin spelling can legitimately come from: `string` is
   /// `std::basic_string`. A record or enum outside both namespaces whose stripped spelling is a
   /// builtin is the contract author's own type colliding with one.
   static constexpr auto std_namespace_prefix = "std::";
