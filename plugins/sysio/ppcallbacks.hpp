...
namespace sysio { namespace cdt {
   ...
   class sysio_ppcallbacks : public PPCallbacks {
      public:
         sysio_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         // LLVM 18: InclusionDirective uses OptionalFileEntryRef instead of const FileEntry*
         virtual void InclusionDirective(
            ...
            auto fe  = sources.getFileEntryRefForID(fid);

            if (fe && !is_angled && llvm::sys::path::filename(fe->getName()) == llvm::sys::path::filename(fn)) {
