...
#include <sysio/base64.hpp>

using namespace sysio;
using namespace std::literals;

SYSIO_TEST_BEGIN(base64enc)
...
SYSIO_TEST_BEGIN(base64dec_extraequals)
...
SYSIO_TEST_BEGIN(base64dec_bad_stuff)
   CHECK_ASSERT( "encountered non-base64 character",
      ([](){
...
SYSIO_TEST_BEGIN(base64urldec_plus_not_allowed)
   CHECK_ASSERT( "encountered non-base64 character",
      ([](){
         base64_decode("YWJjMTIzJCYoKSc+tPUB-n5h"s);
      }));
...
// `/` not valid in base64 url
SYSIO_TEST_BEGIN(base64urldec_slash_not_allowed)
   CHECK_ASSERT( "encountered non-base64 character",
      ([](){
         base64_decode("YWJjMTIzJCYoKSc/tPUB-n5h"s);
      }));
...
SYSIO_TEST_BEGIN(base64urldec_trailing_not_allowed)
   CHECK_ASSERT( "encountered non-base64 character",
      ([](){
...
SYSIO_TEST_BEGIN(base64_cpp_base64_tests)
   //
...
int main(int argc, char* argv[]) {
   ...
   SYSIO_TEST(base64enc);
   SYSIO_TEST(base64urlenc);
   SYSIO_TEST(base64dec);
   SYSIO_TEST(base64urldec);
   SYSIO_TEST(base64dec_extraequals);
   SYSIO_TEST(base64dec_bad_stuff);
   SYSIO_TEST(base64_cpp_base64_tests);
   return has_failed();
}
