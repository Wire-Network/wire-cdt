...
class [[sysio::contract]] simple_tests : public contract {
   public:
      ...
      [[sysio::action("test4")]] 
      void test4(name to) {
         transfer_contract::transfer_action trans("sysio.token"_n, {_self, "active"_n});
      ...
      void on_transfer2(name from, name to, asset quant, std::string memo) {
         check(get_first_receiver() != "sysio.token"_n, "should not be sysio.token");
         print_f("On notify 2 : % % % %", from, to, quant, memo);
      }

      [[sysio::on_notify("*::transfer2")]] 
...
extern "C" void post_dispatch(name self, name original_receiver, name action) {
   ...
   check(self == "sysio"_n, "should only be sysio for action failures");
}
